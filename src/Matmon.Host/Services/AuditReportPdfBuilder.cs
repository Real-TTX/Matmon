...
public sealed class AuditReportPdfBuilder
{
    ...
    private static void ComposeFooter(IContainer container, SummaryReportData data)
    {
        // White-label: the footer credits the partner's product name (with a "powered by Matmon" note kept),
        // else plain "Generated by Matmon".
        var generatedBy = data.Partner?.ProductName is { Length: > 0 } product
            ? $"Generated by {product} · powered by Matmon"
            : "Generated by Matmon";

        container.Row(row =>
        {
            row.RelativeItem().Text(generatedBy).FontSize(8).FontColor(Colors.Grey.Medium);
